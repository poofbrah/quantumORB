...
OUTPUT_DIR = ROOT / "outputs" / "rp_profits_8am_orb_full_window_compare"
WINDOWS = (
    "08:15-09:30",
    "08:30-10:00",
    "09:00-10:30",
...
def main() -> None:
    ...
    for window in WINDOWS:
        summary = run_window(frame, window)
        summaries.append(summary)

    save_outputs(summaries)
    print_summary(len(frame), summaries)
...
def run_window(frame: pd.DataFrame, window: str) -> dict[str, object]:
    ...
    result = BarBacktestEngine(BacktestConfig(position_size=2.0, commission_per_unit=1.40)).run(
        frame,
        setups,
        BacktestRunConfig(strategy_name=f"rp_profits_8am_orb_bos_{window.replace(':', '').replace('-', '_')}_full"),
    )
