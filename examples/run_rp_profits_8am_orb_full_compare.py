...
OUTPUT_DIR = ROOT / "outputs" / "rp_profits_8am_orb_full_compare"
...
def main() -> None:
    ...
    for entry_mode in ENTRY_MODES:
        summary = run_mode(frame, entry_mode)
...
def run_mode(frame: pd.DataFrame, entry_mode: str) -> dict[str, object]:
    ...
    result = BarBacktestEngine(BacktestConfig(position_size=2.0, commission_per_unit=1.40)).run(
        frame,
        setups,
        BacktestRunConfig(strategy_name=f"rp_profits_8am_orb_{entry_mode}_full"),
    )
