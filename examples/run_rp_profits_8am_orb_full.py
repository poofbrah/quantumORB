...
OUTPUT_DIR = ROOT / "outputs" / "rp_profits_8am_orb_full"
...
def main() -> None:
    ...
    setups, diagnostics = detector.detect_with_diagnostics(price_frame)
    labeled = ForwardSetupLabeler(LabelerConfig(horizon_bars=60)).label(price_frame, setups)
    result = BarBacktestEngine(BacktestConfig(position_size=2.0, commission_per_unit=1.40)).run(
        price_frame,
        setups,
        BacktestRunConfig(strategy_name="rp_profits_8am_orb_full"),
    )
